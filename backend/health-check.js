...
const PROD_URL = process.argv[2] || process.env.VERCEL_URL || process.env.API_URL || 'https://d-code-backend.vercel.app';

// Display info about URLs
console.log('\n📊 Testing URLs:');
console.log('   Local URL:', LOCAL_URL);
console.log('   Production URL:', PROD_URL);
console.log('   (You can override production URL by passing it as an argument: node health-check.js https://your-url.com)\n');
...
async function testEndpoint(url, endpoint, description) {
  ...
  try {
    const result = await makeRequest(url, endpoint.path, endpoint.method, endpoint.data);
    ...
    if (result.status >= 200 && result.status < 300) {
      ...
      if (result.data && typeof result.data === 'object') {
        ...
        console.log(`   📄 Response:`, String(result.raw).slice(0, 100) + '...');
      }
      return true;
    } else {
    ...
    if (err.code === 'ECONNREFUSED') {
      console.log(`   💡 Connection refused. Make sure server is running at ${url}`);
    } else if (err.code === 'ENOTFOUND') {
      console.log(`   💡 Host not found. Check URL: ${url}`);
    } else if (err.code === 'ETIMEDOUT') {
      console.log(`   💡 Connection timed out. Server might be slow or unreachable`);
    }
    return false;
  }
}
  ...
  try {
    ...
    if (fs.existsSync(path.join(__dirname, 'test-db-connection.js'))) {
      ...
      return new Promise((resolve) => {
        exec('node test-db-connection.js', { cwd: __dirname }, (error, stdout, stderr) => {
          if (error) {
...
async function main() {
  ...
  if (prodResults) {
    ...
    console.log(`   💡 Set VERCEL_URL or API_URL environment variable to test production`);
  }
  
  console.log(`\n💾 Database Connection: ${dbResult === true ? '✅ Working' : dbResult === false ? '❌ Failed' : '⚠️ Not tested'}`);
  ...
  const hasFailures = localResults.failed > 0 || (prodResults && prodResults.failed > 0) || dbResult === false;
