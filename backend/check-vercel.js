...
function checkEndpoint(url, path) {
  return new Promise((resolve) => {
    ...
    req.on('error', (error) => {
      ...
      console.log(`   (This could indicate the deployment is not running or has an error)`);
      resolve({
        path,
        status: 0,
        success: false,
  ...
  return new Promise((resolve) => {
    ...
    req.on('socket', (socket) => {
      ...
      socket.on('secureConnect', () => {
        console.log(`   TLS handshake completed in ${Date.now() - startTime}ms`);
      });
    });
...
async function checkCommonIssues(url) {
  ...
  if (!url.includes('vercel.app')) {
    console.log(`   ⚠️ URL doesn't appear to be a Vercel deployment (missing vercel.app)`);
  }
  ...
  try {
    ...
    if (res) {
      if (res.statusCode >= 300 && res.statusCode < 400) {
        ...
        console.log('   This may indicate an issue with your Vercel configuration');
      } else {
  ...
  console.log('   2. Ensure your "builds" section points to the correct entry file');
...
async function main() {
  try {
    ...
    console.log(`   ❌ Failed endpoints: ${results.length - successful}/${results.length}`);
